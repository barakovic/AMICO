...
class CylinderHOTmixBall( BaseModel ) :
    ...
    def __init__( self ) :
        ...
        self.d_par  = 1.7E-3                             # Parallel diffusivity [mm^2/s]
        self.Rs     = np.linspace(0.75,3.5,12) * 1E-6    # Radii of the axons [meters]
        self.dPerps_2 = np.linspace(1E-4,2E-3,5)         # HOTmix D_\perp^{(2)} values
        self.dPerps_4 = np.linspace(1e-5,5e-5,5)         # HOTmix D_\perp^{(4)} values
        self.d_ISOs = np.array( [ 3.0E-3 ] )             # Isotropic diffusivitie(s) [mm^2/s]
